<?php
// intranet/admin/unidades_list.php (PHP 5.6) — Responsive: tabla + cards
...
$totalInactivas = 0;
foreach ($rows as $r) {
  if ((int)$r['unidad_activo'] === 1) $totalActivas++; else $totalInactivas++;
}
?>
<!doctype html>
<html lang="es">
<head>
  ...
  <style>
    ...
    .ls-select-input:focus { border-color: var(--blue); box-shadow: 0 0 0 3px var(--blue-subtle); }
    ...
    .ls-table tbody td { padding: 11px 14px; color: var(--text-primary); border-bottom: 1px solid var(--border-2); vertical-align: middle; white-space: nowrap; }
    ...
    .ls-table .td-dir  { font-family: var(--font-mono); font-size: 12px; font-weight: 700; color: var(--amber); letter-spacing: .5px; }
    .ls-table .td-name { white-space: normal; word-break: break-word; }
    .td-actions { display: flex; gap: 4px; align-items: center; }

    /* ── Badge dirección en tabla ── */
    .ls-dir-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 8px; border-radius: 999px; background: var(--amber-subtle); border: 1px solid var(--amber-border); font-size: 11px; font-weight: 600; color: var(--amber); font-family: var(--font-mono); letter-spacing: .3px; }
    ...
    /* ── Status pill ── */
    .ls-pill { display: inline-flex; align-items: center; gap: 5px; padding: 3px 9px; border-radius: 999px; font-size: 11px; font-weight: 600; }
    ...
    .ls-pill-dot { width: 6px; height: 6px; border-radius: 50%; background: currentColor; }
    ...
    .ls-card-top-left { display: flex; flex-direction: column; gap: 5px; min-width: 0; }
    .ls-card-name { font-size: 14px; font-weight: 600; color: var(--text-primary); line-height: 1.4; }
    .ls-card-dir  { display: inline-flex; align-items: center; gap: 5px; padding: 2px 8px; border-radius: 999px; background: var(--amber-subtle); border: 1px solid var(--amber-border); font-size: 11px; font-weight: 600; color: var(--amber); font-family: var(--font-mono); width: fit-content; }
    ...
    .ls-card-dir-full { font-size: 12px; color: var(--text-muted); line-height: 1.4; }
    ...
    <div class="ls-page">
      ...
      <div class="ls-heading">
        <div class="ls-heading-left">
          ...
          <div class="ls-heading-text">
            <h1>Unidades</h1>
            <p>Listado de unidades organizacionales agrupadas por dirección.</p>
          </div>
        </div>
        <div style="display:flex;gap:8px;align-items:center;flex-shrink:0;">
          <a class="btn btn-primary btn-sm" href="unidades_form.php">
      ...
      <div class="ls-filter">
        ...
        <form method="get" class="ls-filter-body">
          ...
          <div class="ls-filter-field">
            <label class="ls-filter-label">Nombre de unidad</label>
            <input class="ls-search-input" name="q" value="<?php echo _sb_h($q); ?>" placeholder="Ej: Recursos Humanos / Obras...">
          </div>
          <div class="ls-filter-actions">
            ...
            <?php if ($q !== '' || $direccionId > 0): ?>
              <a class="btn btn-secondary btn-sm" href="unidades_list.php">Limpiar</a>
            <?php endif; ?>
          </div>
        </form>
      </div>
      ...
      <?php if (empty($rows)): ?>
        <div class="ls-empty">
          ...
          <p>No se encontraron unidades<?php echo ($q !== '' || $direccionId > 0) ? ' con ese filtro' : ''; ?>.</p>
        </div>
      <?php else: ?>
          ...
          <table class="ls-table">
            ...
            <?php foreach ($rows as $r): ?>
              <tr id="row-<?php echo (int)$r['id']; ?>">
                ...
                <td class="td-name"><?php echo _sb_h($r['unidad_nombre']); ?></td>
                <td>
                  <span class="ls-dir-badge">
                    <?php echo _sb_h($r['dir_codigo']); ?>
                  </span>
                  <span style="margin-left:6px;font-size:12px;color:var(--text-muted);"><?php echo _sb_h($r['dir_nombre']); ?></span>
                </td>
                <td>
                  <?php if ((int)$r['unidad_activo'] === 1): ?>
                    <span class="ls-pill active"><span class="ls-pill-dot"></span>Activa</span>
                  <?php else: ?>
                    <span class="ls-pill inactive"><span class="ls-pill-dot"></span>Inactiva</span>
                  <?php endif; ?>
                </td>
                <td>
                  <div class="td-actions">
                    <a class="ls-action-btn is-edit" href="unidades_form.php?id=<?php echo (int)$r['id']; ?>#row-<?php echo (int)$r['id']; ?>">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                      Editar
                    </a>
                    <form method="post" action="unidades_list.php#row-<?php echo (int)$r['id']; ?>">
            ...
            <div class="ls-card" id="card-<?php echo (int)$r['id']; ?>">
              <div class="ls-card-top">
                <div class="ls-card-top-left">
                  <span class="ls-card-name"><?php echo _sb_h($r['unidad_nombre']); ?></span>
                  <span class="ls-card-dir"><?php echo _sb_h($r['dir_codigo']); ?></span>
              ...
              <div class="ls-card-bottom">
                ...
                <div class="ls-card-actions">
                  <a class="ls-action-btn is-edit" href="unidades_form.php?id=<?php echo (int)$r['id']; ?>">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    Editar
                  </a>
                  <form method="post" action="unidades_list.php">
                    <input type="hidden" name="csrf" value="<?php echo _sb_h(csrf_token()); ?>">
                    <input type="hidden" name="toggle_id" value="<?php echo (int)$r['id']; ?>">
                    <?php if ((int)$r['unidad_activo'] === 1): ?>
                      <button class="ls-action-btn is-deactivate" type="submit">
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // 1. Restaurar el scroll exacto de forma instantánea
    ...
    // 2. Aplicar destello visual a la fila/card editada
    ...
    // 3. Interceptar formularios para guardar posición y target
