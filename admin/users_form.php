...
$dirLabel     = $dirHasCodigo ? "CONCAT(d.codigo,' - ',d.nombre)" : "d.nombre";
...
$cargos = array(); $cargosErr = '';
try {
  $st = $pdo->query("
    SELECT c.id, CONCAT($dirLabel,' / ',u.nombre,' / ',c.nombre) AS label
    FROM cargos c JOIN unidades u ON u.id=c.unidad_id JOIN direcciones d ON d.id=u.direccion_id
    $whereSql
    ORDER BY ".($dirHasCodigo ? "d.codigo," : "")." d.nombre, u.nombre, c.nombre
  ");
  $cargos = $st ? $st->fetchAll(PDO::FETCH_ASSOC) : array();
} catch (Exception $e) { $cargos = array(); $cargosErr = $e->getMessage(); }

/* ── Estado del formulario ──────────────────────────────── */
$err = ''; $ok = '';
$defaults = array(
  'rut'=>'','nombres'=>'','apellidos'=>'','email'=>'','telefono'=>'','direccion_dom'=>'',
  'activo'=>1,'is_superadmin'=>0,'roles'=>array(),'password'=>'','password2'=>'',
  'cargo_id'=>0,'cargo_tipo'=>'titular','fecha_desde'=>date('Y-m-d'),'fecha_hasta'=>''
);
$data = $defaults;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($data['rut'] === '' || !valid_rut_format($data['rut']))      $err = 'RUT inválido (ej: 12345678K).';
  elseif ($data['nombres'] === '' || $data['apellidos'] === '')     $err = 'Nombres y apellidos son obligatorios.';
  elseif ($data['password'] === '' || strlen($data['password']) < 3)$err = 'La clave debe tener al menos 3 caracteres.';
  elseif ($data['password'] !== $data['password2'])                 $err = 'Las claves no coinciden.';
  elseif ($data['email'] !== '' && !filter_var($data['email'], FILTER_VALIDATE_EMAIL)) $err = 'Email inválido.';
  elseif (!in_array($data['cargo_tipo'], array('titular','subrogante','suplente'), true)) $err = 'Tipo de cargo inválido.';
  else {
  ...
  if ($err === '') {
    ...
    if (!$hash) { $err = 'Error al generar contraseña.'; }
    else {
      $pdo->beginTransaction();
      try {
        $ins = $pdo->prepare("
          INSERT INTO funcionarios (rut,nombres,apellidos,email,telefono,direccion_dom,clave_hash,activo,is_superadmin)
          VALUES (?,?,?,?,?,?,?,?,?)
        ");
        $ins->execute(array(
          $data['rut'],$data['nombres'],$data['apellidos'],
          $data['email']!==''?$data['email']:null,$data['telefono']!==''?$data['telefono']:null,
          $data['direccion_dom']!==''?$data['direccion_dom']:null,
          $hash,(int)$data['activo'],(int)$data['is_superadmin']
        ));
        $newId = (int)$pdo->lastInsertId();
        if (!empty($data['roles'])) {
          $insR = $pdo->prepare("INSERT INTO funcionario_roles (funcionario_id,rol_id) VALUES (?,?)");
          foreach ($data['roles'] as $rid) { $rid=(int)$rid; if ($rid>0) $insR->execute(array($newId,$rid)); }
        }
        if ($data['cargo_id'] > 0) {
          ...
          $insC = $pdo->prepare("INSERT INTO funcionario_cargos (funcionario_id,cargo_id,tipo,fecha_desde,fecha_hasta,activo,observacion) VALUES (?,?,?,?,?,1,?)");
          $insC->execute(array($newId,(int)$data['cargo_id'],$data['cargo_tipo'],$data['fecha_desde'],($data['fecha_hasta']!==''?$data['fecha_hasta']:null),'Creación inicial'));
        }
        $pdo->commit();
        if (function_exists('acl_cache_reset')) acl_cache_reset();
        $ok   = 'Usuario creado (ID: '.$newId.').';
        $data = $defaults;
      } catch (Exception $e) { $pdo->rollBack(); $err = 'Error: '.$e->getMessage(); }
    }
  }
}
?>
